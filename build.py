...
class Build:
    ...
    def android_sdk_root(self, root: str = None, ndk_root: str = None):
        ...
        clang_dir = llvm_root / 'lib' / 'clang'

        ndk_dir.mkdir(parents=True, exist_ok=True)
        if ndk_version_dir.is_symlink():
        ...
        for child in ndk_root.iterdir():
            if child.name == 'toolchains':
                continue
            ensure_symlink(ndk_version_dir / child.name, child)

        llvm_stub = ndk_version_dir / 'toolchains' / 'llvm' / 'prebuilt' / 'linux-x86_64'
        for child in llvm_root.iterdir():
            if child.name == 'lib':
                continue
            ensure_symlink(llvm_stub / child.name, child)

        lib_root = llvm_root / 'lib'
        lib_stub = llvm_stub / 'lib'
        for child in lib_root.iterdir():
            if child.name == 'clang':
                continue
            ensure_symlink(lib_stub / child.name, child)

        clang_stub = lib_stub / 'clang'
        clang_stub.mkdir(parents=True, exist_ok=True)
        for child in clang_dir.iterdir():
            if child.is_dir():
                ensure_symlink(clang_stub / child.name, child)

        self.ensure_android_ndk_clang_alias(clang_dir, clang_stub)
        return str(sdk_root)

    def ensure_android_ndk_clang_alias(self, clang_dir: Path, clang_stub: Path, expected_version: str = '19'):
        if not clang_dir.exists():
            return

        expected_dir = clang_stub / expected_version
        ...
        ensure_symlink(expected_dir, actual_versions[-1])
