...
echo "║     Flutter ${FLUTTER_VERSION} | Android SDK ${ANDROID_SDK_VERSION}                  ║"
echo "╚═══════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
echo -e "${GREEN}[1/6]${NC} Updating packages..."
pkg update -y
pkg upgrade -y

echo -e "${GREEN}[2/6]${NC} Installing dependencies..."
pkg install -y x11-repo
pkg install -y openjdk-17 git wget curl

echo -e "${GREEN}[3/6]${NC} Downloading Flutter SDK..."
cd ~
...
echo -e "${GREEN}[4/6]${NC} Downloading Android SDK..."
...
echo -e "${GREEN}[5/6]${NC} Installing packages..."
dpkg -i "flutter_${FLUTTER_VERSION}_aarch64.deb" || true
dpkg -i "android-sdk_${ANDROID_SDK_VERSION}_aarch64.deb" || true
apt --fix-broken install -y

echo -e "${GREEN}[6/6]${NC} Configuring environment..."
...
echo ""
echo "4. Create and build your first app:"
echo -e "   ${BLUE}flutter create myapp && cd myapp${NC}"
echo -e "   ${BLUE}flutter build apk --release --target-platform android-arm64${NC}"
echo ""
